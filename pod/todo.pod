...
=head3 ICAL file parser

ICal files are conveniently available, for example from the YAPC::Europe
program and provide a quick start to a searchable corpus. Also see
L<Cal::DAV>, which can parse the calendar entries.
